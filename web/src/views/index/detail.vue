          ...
          <div class="thing-infos-view">
            <div class="thing-infos">
              ...
              <div v-else>
                <h1 class="thing-name">{{ detailData.title }}</h1>
                <div class="translators flex-view" style="">
                  <span style="white-space: pre-line">{{ detailData.description }}</span>
                </div>
              </div>
            </div>
          ...
          <div class="main-content">
            ...
            <div class="thing-comment" :class="selectTabIndex > 0 ? '' : 'hide'">
              ...
              <div class="publish flex-view">
                <img v-if="userInfo.avatarURL" :src=userInfo.avatarURL class="mine-img">
              ...
              <div class="comments-list">
                <div class="comment-item" v-for="item in commentData">
                  <div class="flex-item flex-view">
                    <img v-if="item.avatar" :src="item.avatar" class="avator">
...
const getUserInfo = () => {
  ...
  userDetailApi({ userId: userId }).then(res => {
    if (res.data.avatar) {
...
const getThingDetail = () => {
  detailApi({ id: thingId.value, sort: sortId.value }).then(res => {
    ...
    detailData.value.cover = BASE_URL + '/api/staticfiles/cover/' + detailData.value.cover
  }).catch(err => {
